...
- [x] 0.1 Im Feature-Worktree `pnpm add sharp` ausführen und `package.json` +
...
- [x] 1.1 Socket-Integrationstests in einer neuen Fog-Socket-Suite: 14 der 15 Szenarien
...
- [x] 1.2 Aushängen-Szenario „Aushängen löscht aufgedeckte Zellen und Bereiche" in derselben
      Suite (Route `DELETE /api/sessions/:id/maps/:instanceId`, danach `count` je Tabelle)
- [x] 1.3 REST-Integrationstests in einer neuen Kartenbild-Suite: die 7 Szenarien
...
- [x] 1.4 Bestehende Bildroute: das Szenario „Mitglied erhält das Bild der aktiven Karte"
...
- [x] 1.5 Komponententests in einer neuen Fog-UI-Suite: die 16 Szenarien „Fog-Ansicht im
...
- [x] 1.6 Bestehende Kartenansichts-Szenarien: „Raum mit aktiver Karte zeigt die
...
- [x] 2.1 `prisma/schema.prisma`: `MapInstance.fogVersion` (`Int`, Default 0) und die
...
- [x] 3.1 `src/shared/fog.ts` (neu): Konstanten, `CellSchema`, `CellListSchema`,
...
- [x] 4.1 `src/server/map/storage.ts`: `imageSize` über `sharp` (design.md D5);
      `src/server/map/rules.ts`: `findViewableMap` entfernen; `src/server/map/routes.ts`:
      `GET /api/maps/:id/image` über `findOwnMap`
- [x] 4.2 `src/server/session/fog.ts` (neu): `toFogState`, `loadFog`, `loadFogFor`,
...
- [x] 4.3 `src/server/session/socket.ts`: `fog` im Enter-Ack, `registerFogHandlers` in der
...
- [x] 4.4 `src/server/session/map-image.ts` (neu): `renderMaskedImage`,
...
- [x] 5.1 `src/client/session/socket.ts`: `setFog`, `createFogArea`, `deleteFogArea`,
      `on('fog')`, `wireEventFor` (design.md D7); `src/client/session/fog-api.ts` (neu):
      `sessionMapImageUrl`; verifizieren mit `pnpm typecheck:src`
- [x] 5.2 `src/client/map/canvas.ts`: Optionen `fog`, `tool`, `selection`,
...
- [x] 5.3 `src/client/session/FogPanel.tsx` (neu): Fog-Verwaltung genau nach der
      Schnittstellentabelle in design.md D8; lokaler Zustand nur für das Namensfeld
- [x] 5.4 `src/client/session/SessionRoom.tsx`: Zustand `fog`/`fogTool`/`fogSelection`/
...
- [x] 6.3 Change nach `openspec/changes/archive/YYYY-MM-DD-add-fog-of-war/` verschieben
