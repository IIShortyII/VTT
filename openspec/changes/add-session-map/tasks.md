...
- [ ] 1.1 Integrationstests für die 9 REST-Szenarien der Requirements „Instanzrouten
...
- [ ] 1.2 Socket-Integrationstests für die 6 Szenarien der Requirement „Aktive Karte
...
- [ ] 1.3 Komponententests für die 11 Szenarien der Requirement „Kartenansicht im Raum"
...
- [ ] 2.1 `prisma/schema.prisma`: Modell `MapInstance` mit `@@unique([sessionId, mapId])`
...
- [ ] 3.1 `src/shared/session-map.ts`: `MapInstanceSchema`, `ActiveMapSchema`,
...
- [ ] 3.2 `src/shared/session.ts`: `EnterAck` um `map: ActiveMap | null` erweitern
      (design.md D5); verifizieren mit `pnpm typecheck:src`

## 4. Server (implementer)

- [ ] 4.1 `src/server/session/active-map.ts`: `toActiveMap`, `loadActiveMap`,
      `emitActiveMap`, `broadcastMapChanged` (design.md D4); verifizieren mit
      `pnpm typecheck:src`
- [ ] 4.2 `src/server/session/maps.ts`: `findLedSession` (Spielleiter-Mitgliedschaft in der
...
- [ ] 4.3 `src/server/session/socket.ts`: Ereignis `session:activate-map` (zod-Parse →
...
- [ ] 4.4 `src/server/map/rules.ts`: `findViewableMap` (Besitzer oder Mitglied einer
...
- [ ] 4.5 `src/server/core/app.ts`: `registerSessionMapRoutes(app, { prisma, clock, io,
      presence })` und `io` an `registerMapRoutes` (design.md D2); verifizieren mit
      `pnpm typecheck:src` und `pnpm lint`

## 5. Client (implementer)

- [ ] 5.1 `src/client/session/socket.ts`: `activateMap(sessionId, instanceId | null)` mit
      Acknowledgement und `on('map', …)` für `session:map` (design.md D7); verifizieren mit
      `pnpm typecheck:src`
- [ ] 5.2 `src/client/session/maps-api.ts`: `listSessionMaps`, `mountMap`, `unmountMap` mit
...
- [ ] 5.3 `src/client/session/MapPanel.tsx`: Überschrift `Karten`, Liste der Instanzen mit
...
- [ ] 5.4 `src/client/session/SessionRoom.tsx`: `map` aus dem Enter-Ack im Zustand,
...
- [ ] 6.1 Gate grün (Typecheck, Lint, komplette Jest-Suite inkl. der bestehenden Tests),
      Review „ok"
- [ ] 6.2 App-Test durch den Menschen (zwei Browser, Spielleiter und Spieler): Karte aus der
...
- [ ] 6.3 Change nach `openspec/changes/archive/YYYY-MM-DD-add-session-map/` verschieben
