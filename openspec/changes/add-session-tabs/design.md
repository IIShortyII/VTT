...
| Reiterpanel | sichtbar: `getByRole('tabpanel', { name: 'Tokens' })` (Name über `aria-labelledby`); versteckt: `queryByRole('tabpanel', { name: 'Tokens' })` ist `null` und `getByRole('tabpanel', { name: 'Tokens', hidden: true })` trägt das Attribut `hidden`; „nicht im sichtbaren DOM-Pfad" = die Standardabfragen (`queryByRole('heading', { name: 'Tokens' })`, `queryByRole('list', …)`) liefern `null`, weil Testing Library `hidden`-Teilbäume ausschließt |
