...
- [ ] 1.1 Socket-Integrationstests für die 4 Szenarien „Token anlegen", die 3 Szenarien
...
- [ ] 1.2 Komponententests für die 8 Szenarien „Tokenansicht im Raum" (eigene
...
- [ ] 2.1 `prisma/schema.prisma`: Modell `Token` mit `onDelete: Cascade` zur Instanz und
...
- [ ] 3.1 `src/shared/token.ts`: `TOKEN_ICONS`, `TokenIconSchema`, Konstanten, `TokenSchema`,
      ...
      `pnpm typecheck:src`
- [ ] 3.2 `src/shared/session.ts`: `EnterAck` um `tokens: Token[]` erweitern (design.md D4);
      verifizieren mit `pnpm typecheck:src`

## 4. Server (implementer)

- [ ] 4.1 `src/server/session/tokens.ts`: `toToken`, `loadTokens`, `emitTokens`,
...
- [ ] 4.2 `src/server/session/socket.ts`: `registerTokenHandlers` in der
...
- [ ] 4.3 `src/server/session/maps.ts`: nach `emitActiveMap(io, id, null)` beim Aushängen
...
- [ ] 5.1 `src/client/session/socket.ts`: `createToken`, `moveToken`, `removeToken` mit
      Acknowledgement und `on('tokens', …)` für `session:tokens` in `wireEventFor`
      (design.md D5); verifizieren mit `pnpm typecheck:src`
- [ ] 5.2 `src/client/map/canvas.ts`: `tokens` und optionales `onTokenMove` in den
...
- [ ] 5.3 `src/client/session/TokenPanel.tsx`: Überschrift `Tokens`, Formular nach der
...
- [ ] 5.4 `src/client/session/SessionRoom.tsx`: `tokens` aus dem Enter-Ack im Zustand,
...
- [ ] 6.1 Gate grün (Typecheck, Lint, komplette Jest-Suite inkl. der bestehenden Tests),
      Review „ok"
- [ ] 6.2 App-Test durch den Menschen (zwei Browser, Spielleiter und Spieler): Karte
...
- [ ] 6.3 Change nach `openspec/changes/archive/YYYY-MM-DD-add-session-tokens/` verschieben
