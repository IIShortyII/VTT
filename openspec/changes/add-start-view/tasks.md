...
- [x] 1.1 Komponententests für die Startansicht (`tests/ui-start.unit.test.tsx`,
...
- [x] 1.2 Bestehende Tests zu den MODIFIED-Szenarien umstellen: `game-session`
...
- [x] 2.1 `src/client/app/Hero.tsx` (neu) genau nach design.md D1: Overline `Deine Runde`
      (Striche nur per CSS), `<h1 className="hero-title">`, Subline, optionale
      Aktionszeile
- [x] 2.2 `src/client/session/session-status.ts` (neu) nach design.md D3:
      `SESSION_STATUS_PRESENTATION` (Text, Icon, Varianten-Klasse je Zustand) und
      `ROLE_LABELS`
- [x] 2.3 `src/client/session/SessionList.tsx` nach design.md D4: Hero mit `Sitzung leiten`
      ...
      entfernt
- [x] 2.4 `src/client/app/App.tsx` nach design.md D2: anonymer Zweig mit `Hero` und
      ...
      `field-label`
- [x] 2.5 `src/client/app/AppShell.tsx` nach design.md D6: `<Icon name="back" />` statt
      `‹`, zugänglicher Name `Zurück` unverändert
- [x] 2.6 `src/client/app/theme.css`: Abschnitt „Startansicht" mit den zwölf Selektoren
...
- [x] 3.3 Change nach `openspec/changes/archive/YYYY-MM-DD-add-start-view/` verschieben
