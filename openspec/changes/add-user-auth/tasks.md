...
- [ ] 1.1 `@fastify/cookie` durch den Menschen installieren lassen (§5.2, freigegeben) und
      verifizieren, dass `pnpm install` durchläuft und das Paket in `package.json` steht
- [ ] 1.2 `prisma/schema.prisma` um `User`, `Account` und `Session` nach design.md D2
      ergänzen; verifizieren mit `pnpm db:generate` (Client wird ohne Fehler erzeugt).
      Migration nicht ausführen (§5.1)
- [ ] 1.3 `src/server/core/config.ts` anlegen: `DATABASE_URL` und der `secure`-Schalter fürs
...
- [ ] 1.4 `src/server/core/clock.ts` anlegen (`type Clock = () => Date`, Default `() => new
      Date()`); verifizieren über den Typecheck und die spätere Verwendung in 3.2
- [ ] 1.5 `src/server/core/prisma.ts` anlegen (eine geteilte `PrismaClient`-Instanz);
      verifizieren über den Typecheck

## 2. Gemeinsamer Vertrag

- [ ] 2.1 `src/shared/auth.ts` mit den zod-Schemas für Registrierung und Anmeldung
...
- [ ] 3.1 `src/server/auth/rules.ts`: `hashPassword` und `verifyPassword` über die
...
- [ ] 3.2 In derselben Datei die Sitzungsarithmetik: Ablaufzeitpunkt aus einer übergebenen
...
- [ ] 3.3 `normalizeEmail` (trimmen, kleinschreiben) ergänzen und überall dort verwenden, wo
...
- [ ] 4.1 `src/server/core/app.ts`: Fastify-Instanz als Funktion, die eine fertig
...
- [ ] 4.2 `src/server/auth/session.ts`: Sitzung anlegen (opake ID nach D4), Sitzung aus dem
...
- [ ] 4.3 `POST /api/auth/register`: zod-Validierung an der Grenze, Konflikt bei vergebener
...
- [ ] 4.4 `POST /api/auth/login`: Konto suchen, Passwort prüfen, bei unbekannter E-Mail gegen
...
- [ ] 4.5 `GET /api/auth/me` und `POST /api/auth/logout`: Nutzer aus der Sitzung, bzw.
...
- [ ] 4.6 Sicherstellen, dass keine Antwort Hash, Salt oder Hash-Parameter enthält — der
...
- [ ] 4.7 Zentraler Fehler-Handler in `app.ts`: ein fehlgeschlagener Request antwortet
...
- [ ] 4.8 `src/server/index.ts`: Serverstart auf dem konfigurierten Port; verifizieren mit
      `pnpm dev:server` (Server hört, `/api/auth/me` antwortet mit 401)

## 5. Client

- [ ] 5.1 `index.html` und `vite.config.ts` anlegen — React-Plugin und Dev-Proxy für `/api`
      auf den Fastify-Port (D7); verifizieren mit `pnpm dev:client` (Seite lädt, eine
      `/api`-Anfrage erreicht den Server) und `pnpm build`
- [ ] 5.2 `src/client/auth/api.ts`: die vier Aufrufe gegen `/api/auth/*` mit
      `credentials: 'include'`, Antworten gegen die Schemas aus `shared/auth.ts` geprüft;
      verifizieren über den Typecheck
- [ ] 5.3 `src/client/app/App.tsx` mit den drei Zuständen `unbekannt` / `anonym` /
      `angemeldet` (D7); verifiziert durch das Szenario „Ohne Anmeldung erscheint das
      Anmeldeformular"
- [ ] 5.4 `LoginForm` und `RegisterForm` mit nativen Formularelementen (D8), inklusive
      Anzeige eines vom Server gemeldeten Fehlschlags; verifiziert durch „Fehlgeschlagene
      Anmeldung wird angezeigt"
- [ ] 5.5 `src/client/main.tsx` als React-Einstieg; verifizieren mit `pnpm build` und im
      App-Test aus 6.2

## 6. Abschluss

- [ ] 6.1 Gate grün: `pnpm typecheck`, `pnpm lint` und die vollständige Jest-Suite über den
      Orchestrator; danach reviewer (§3.2/§3.3)
- [ ] 6.2 Nach „ok" des reviewers die App lokal starten (`pnpm dev`), dem Menschen Link und
      Änderungsliste zum manuellen Test vorlegen und auf ausdrückliche Freigabe warten (§3.4)
- [ ] 6.3 Nach der Freigabe den Change ins `archive/` verschieben — im selben Branch und
