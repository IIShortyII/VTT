...
- [x] 1.1 Integrationstests für die sieben Szenarien der Requirement „Sperre nach
...
- [x] 1.2 Integrationstests für die sechs Szenarien der Requirement „Passwortänderung"
      (`POST /api/auth/password`); verifizieren, dass sie rot sind (Route existiert nicht →
      `404`, nicht der erwartete Status)
- [x] 1.3 Komponententests für die drei Szenarien der Requirement „Passwortänderung in der
...
- [x] 2.1 `prisma/schema.prisma`: `Account` um `failedLoginCount Int @default(0)` und
...
- [x] 3.1 `src/server/auth/rules.ts`: `LOCKOUT_THRESHOLD`, `LOCKOUT_DURATION_MS`,
...
- [x] 3.2 `src/server/auth/routes.ts`, `POST /api/auth/login`: Hash immer prüfen, danach
...
- [x] 3.3 `src/shared/auth.ts`: `ChangePasswordInputSchema` (`currentPassword` in Login-Form,
      `newPassword` nach `PasswordSchema`) und Typ (design.md D3)
- [x] 3.4 `src/server/auth/routes.ts`, `POST /api/auth/password`: Sitzung auflösen (`401`),
...
- [x] 4.1 `src/client/auth/api.ts`: `changePassword(input)` mit lokaler `safeParse`-Vorprüfung
      und Durchreichen der `ErrorOutput`-Form (design.md D5)
- [x] 4.2 `src/client/auth/ChangePasswordForm.tsx`: zwei Passwortfelder, Absende-Button,
...
- [x] 4.3 `src/client/app/App.tsx`: Formular in der angemeldeten Ansicht rendern, ohne
      Zustandswechsel in `App` (design.md D5); verifiziert durch die Szenarien der Requirement
      „Passwortänderung in der Oberfläche"

## 5. Abschluss

- [x] 5.1 Gate grün: `pnpm typecheck`, `pnpm lint` und die vollständige Jest-Suite über den
      Orchestrator; danach reviewer (§3.2/§3.3) — der reviewer prüft die erzeugte Migrations-SQL
      gegen design.md D1
- [x] 5.2 Vor dem App-Test: `.env` aus dem Hauptrepo in den Worktree kopieren; der Mensch
      ...
      warten (§3.4)
- [x] 5.3 Nach der Freigabe den Change ins `archive/` verschieben und das Delta in
