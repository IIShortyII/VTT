...
- [x] 1.1 `openspec/specs/user-auth/spec.md` aus dem archivierten Delta von `add-user-auth`
...
- [x] 2.1 `tests/user-auth.integration.test.ts`: Wegwerf-DB im `beforeAll` mit
...
- [x] 2.2 Integrationstests für die geänderten und neuen Szenarien aus dem Delta
...
- [x] 2.3 `tests/auth-ui.unit.test.tsx`: Komponententests für `Server beim Start nicht
...
- [x] 3.1 `src/server/auth/routes.ts`: Cookie-Helfer leitet `maxAge` aus `expiresAt` und der
...
- [x] 3.2 `src/server/auth/session.ts`: `renewed` aus `ResolvedSession` entfernen, `expiresAt`
      bleibt (design.md D1); verifizieren mit `pnpm typecheck:src`
- [x] 3.3 `GET /api/auth/me`: im `401`-Zweig nach `resolveSession` das Cookie entwerten
      (design.md D2); verifiziert durch „Abgelaufene Sitzung gilt nicht mehr"
- [x] 3.4 `POST /api/auth/login`: Zweigwahl nach design.md D3 als reine Funktion — `401` nur,
...
- [x] 4.1 `src/client/app/App.tsx`: Mount-Effekt fängt Fehler, fällt auf `anonym` zurück und
      zeigt einen Hinweis (design.md D4); verifiziert durch „Server beim Start nicht
      erreichbar"
- [x] 4.2 `src/client/app/App.tsx`: Abmeldung fängt Fehler, bleibt `angemeldet` und zeigt die
...
- [x] 5.2 Nach „ok" des reviewers die App lokal starten (`pnpm dev`), dem Menschen Link und
      Änderungsliste zum manuellen Test vorlegen und auf ausdrückliche Freigabe warten (§3.4)
- [x] 5.3 Nach der Freigabe den Change ins `archive/` verschieben — im selben Branch und
