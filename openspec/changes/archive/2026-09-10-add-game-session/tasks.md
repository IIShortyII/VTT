...
- [x] 1.1 Integrationstests für die acht REST-Szenarien der Requirements „Spielsitzung
...
- [x] 1.2 Integrationstests für die 22 Socket-Szenarien der Requirements „Verbindung und
...
- [x] 1.3 Komponententests für die sechs Szenarien der Requirement „Sitzungsoberfläche"
...
- [x] 2.1 `prisma/schema.prisma`: Modelle `GameSession` und `Membership` sowie die
...
- [x] 3.1 `src/shared/session.ts`: `GameSessionStatusSchema`, `MemberRoleSchema`,
...
- [x] 4.1 `src/server/session/rules.ts`: `generateCode(randomInt)` aus dem Alphabet,
      ...
      `pnpm typecheck:src`
- [x] 4.2 `src/server/session/presence.ts`: Klasse `Presence` mit `enter`/`leave`/`isOnline`/
...
- [x] 4.3 `src/server/session/authorize.ts`: `authorizeAction(deps, socket, gameSessionId,
...
- [x] 4.4 `src/server/session/room.ts`: `buildParticipants(prisma, presence, gameSessionId)`
...
- [x] 4.5 `src/server/session/routes.ts`: `POST /api/sessions` (`401` ohne Sitzung, `400` +
...
- [x] 4.6 `src/server/session/socket.ts`: Middleware (Cookie vorhanden, sonst Ablehnung),
...
- [x] 4.7 `src/server/core/app.ts`: `new Server(app.server, { serveClient: false })`,
...
- [x] 5.1 `vite.config.ts`: Proxy `'/socket.io'` mit `ws: true` auf den Fastify-Port
      (design.md D9); verifizieren durch Sichtprüfung der Konfiguration und im App-Test
      (WebSocket statt Polling im Netzwerk-Tab)
- [x] 5.2 `src/client/session/api.ts`: `listSessions`, `createSession`, `joinSession` mit
...
- [x] 5.3 `src/client/session/socket.ts`: Fassade über `socket.io-client` (`autoConnect:
...
- [x] 5.4 `src/client/session/SessionList.tsx`: Liste mit Name, Rolle, Zustand und „Betreten";
...
- [x] 5.5 `src/client/session/SessionRoom.tsx`: `enter` beim Mounten, Anzeige von Name,
...
- [x] 5.6 `src/client/app/App.tsx`: angemeldete Ansicht rendert `SessionList` bzw.
...
- [x] 6.1 Gate grün (Typecheck, Lint, komplette Jest-Suite inkl. der bestehenden
      `user-auth`-Tests), Review „ok"
- [x] 6.2 App-Test durch den Menschen: zwei Browser-Profile (Spielleiter, Spieler) —
...
- [x] 6.3 Change nach `openspec/changes/archive/YYYY-MM-DD-add-game-session/` verschieben
