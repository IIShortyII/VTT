...
- [x] 1.1 Bestehende Tests auf den neuen Vertrag umstellen: jede Registrierung in
...
- [x] 1.2 Integrationstests für die vier neuen und die drei geänderten REST-Szenarien der
...
- [x] 1.3 Komponententests für die zwei Szenarien der Requirement „Registrierungsoberfläche"
...
- [x] 1.4 Socket-Integrationstests für die acht Szenarien der Requirement „Alias pro
...
- [x] 1.5 Komponententests für die drei neuen Szenarien der Requirement „Sitzungsoberfläche"
...
- [x] 2.1 `prisma/schema.prisma`: `User.username String`, `User.usernameKey String @unique`,
...
- [x] 3.1 `src/shared/auth.ts`: `USERNAME_MIN_LENGTH`/`USERNAME_MAX_LENGTH`, `UsernameSchema`
...
- [x] 3.2 `src/shared/session.ts`: `ParticipantSchema` ohne `email`, mit `username` und
...
- [x] 4.1 `src/server/auth/rules.ts`: reine Funktion `usernameKey(username)` =
      `normalize('NFKC').toLowerCase()`; `toUserOutput` liefert `username` mit; verifizieren
      mit `pnpm typecheck:src`
- [x] 4.2 `src/server/auth/routes.ts`: Registrierung legt `username` und `usernameKey` an;
...
- [x] 4.3 `src/server/session/room.ts`: `buildParticipants` selektiert `user.username` statt
...
- [x] 4.4 `src/server/session/socket.ts`: Handler `handleAlias` für `session:alias` nach dem
...
- [x] 5.1 `src/client/auth/RegisterForm.tsx`: Feld `username` (Label „Nutzername", `id`
...
- [x] 5.2 `src/client/session/socket.ts`: Fassade um `alias(sessionId, alias): Promise<AliasAck>`
      erweitern (Muster `transition`); verifizieren mit `pnpm typecheck:src`
- [x] 5.3 `src/client/session/SessionRoom.tsx`: Prop `currentUserId`; Teilnehmer über
...
- [x] 5.4 `pnpm typecheck:src` und `pnpm lint` grün; `pnpm build` läuft durch

## 6. Abschluss (Session, nach menschlicher App-Freigabe)

- [x] 6.1 Absatz „Drahtformat" unter „Begriffe" in `openspec/specs/game-session/spec.md`:
...
- [x] 6.2 OpenSpec-Change nach `openspec/changes/archive/YYYY-MM-DD-add-username-and-alias/`
