...
- [x] 1.1 Socket-Integrationstests für die 12 Szenarien „Sichtbarkeit von Tokens im Fog" in
...
- [x] 1.2 Die zwei neuen Szenarien „Verborgenes Token ist beim Bewegen nicht
...
- [x] 1.3 Bestehende Token-Socket-Suite auf die Konvention „vollständig aufgedeckt"
...
- [x] 2.1 `src/shared/token.ts`: `tokenCells(token, gridType)` und
...
- [x] 3.1 `src/server/session/tokens.ts`: `loadVisibilityContext`, `loadTokenInventory`,
...
- [x] 3.2 `src/server/session/tokens.ts`: Sichtbarkeitsprüfung in `handleMoveToken` und
...
- [x] 3.3 `src/server/session/fog.ts`: `broadcastTokens` nach `broadcastFog` in
...
- [x] 4.1 Gate grün (Typecheck, Lint, komplette Jest-Suite inkl. der bestehenden Tests),
      Review „ok"
- [x] 4.2 App-Test durch den Menschen (Dev-DB ist seit #16 migriert, keine neue Migration;
...
- [x] 4.3 Change nach `openspec/changes/archive/YYYY-MM-DD-add-token-fog-visibility/`
