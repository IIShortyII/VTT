...
- [x] 1.1 `.harness/tests/setup.ts` anlegen, das `HARNESS_BOARD=off` setzt, und in
...
- [x] 1.2 `.harness/tests/board.test.ts` anlegen mit den Szenarien aus
      ...
      sind rot
- [x] 1.3 `.harness/tests/orchestrator.test.ts` um die Szenarien ergänzen, die die
      ...
      sind rot
- [x] 1.4 Prüfen, dass das Rot aus dem richtigen Grund kommt (§3.1): die Fehlschläge nennen
...
- [x] 2.1 `.harness/board.ts` anlegen: ID-Tabelle nach design.md D8 (Projekt, Statusfeld,
...
- [x] 2.2 Auflösung des Item-Handles nach design.md D5 ergänzen (GraphQL-Query, Auswahl des
...
- [x] 2.3 `setBoardStatus` mit Statuswort→Option-Tabelle, Mutation aus festen Konstanten,
...
- [x] 2.4 Fehlerkapselung und Protokoll nach design.md D6 ergänzen (nie werfen, `[board]` auf
...
- [x] 3.1 Aktions→Status-Tabelle in `emit()` ergänzen (`invoke-implementer` →
      ...
      punkte je Aktion
- [x] 3.2 `gate()` setzt „Gate + Review" als erste Anweisung, vor Typecheck/Lint/Testlauf
      (D3); verifizieren mit dem Testfall „Der Gate-Start setzt Gate + Review"
- [x] 3.3 `confirmRed()` setzt „Test rot" ausschließlich im Zweig `red === true` (§3.1);
      verifizieren mit den beiden Testfällen zum bestätigten und zum nicht bestätigten Rot
- [x] 3.4 `cleanup()` setzt „Fertig" nur bei geschlossenem Issue, mit Abfrage des
...
- [x] 3.5 Verb `board <issue> <status>` im Dispatch ergänzen — ohne Zugriff auf `status.json`
...
- [x] 4.1 `AGENTS.md`: unter „Kommandos" das Verb `pnpm harness board <issue> <status>`
...
- [x] 4.2 `.claude/skills/feature-loop/SKILL.md`: die Zeile ergänzen, dass die Session zu
...
- [x] 5.2 Gegenprobe am echten Board: einen Statuswechsel für Issue #26 auslösen und das
