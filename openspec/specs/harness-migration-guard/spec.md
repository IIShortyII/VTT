# harness-migration-guard Specification

## Purpose
Beschreibt, wogegen die Migrationssperre des PreToolUse-Guards prüft: welche Quelle für
`DATABASE_URL` gilt (eine Inline-Zuweisung im Kommandotext vor der Umgebung des Hook-Prozesses),
welche Zuweisungsformen erkannt werden, dass der Wegwerf-Charakter am ganzen Wert geprüft wird,
und dass alles Unerkannte blockt.

Die Sperre ist die einzige mechanische Durchsetzung von `constitution.md` §5.1 und §6.1
(Migrationen nur gegen die Wegwerf-DB, nie gegen eine produktive Zielumgebung). Ihr einziger
teurer Fehler ist fail-open — und der geprüfte Text stammt vom Agenten, nicht vom Menschen.
Jede Regel hier ist deshalb so geschnitten, dass ein Zweifel blockt, nie durchlässt.
`harness-guard-hook` beschreibt daneben nur, *ob* der Hook überhaupt läuft.
