# player-bar Specification

## Purpose

Die Leiste am Kopf der Raumansicht für die Rolle `spieler`: Identität (Name der
Spielsitzung, Rollen-Pille), Zustandspille, die drei On-Demand-Trigger `Tokens`,
`Anmerkungen`, `Teilnehmer` mit dem Tokens-Badge und die Verbindungsanzeige. Sie ersetzt
für den Spieler die Session-Bar (`session-bar`) und die Reiterliste (`session-tabs`): die
Karte ist sein einziger dauerhaft sichtbarer Bereich, alles Weitere öffnet on demand in
Modals (`ui-dialog`). Die Leiste zeigt ausschließlich, was der Server gemeldet hat — keinen
Sitzungscode, keine Steuerung (`constitution.md` §9.1/§9.2); das Badge folgt dem vom Server
gefilterten Tokenbestand, nie einer lokalen Schätzung.
