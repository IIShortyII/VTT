...
- **Aktionen**: in der angemeldeten Startansicht die Schaltflächen `Sitzung leiten`
  ...
  `Sitzung leiten` und `Beitreten` öffnen je einen Dialog (`ui-dialog`) und tragen
  `aria-haspopup="dialog"`.
- **Erstellen-Formular**: das Formular mit dem zugänglichen Namen `Neue Spielsitzung` im
  Dialog gleichen Namens (der Dialogtitel ist die Überschrift der Ebene 2), Eingabefeld
  `Name` (`name="name"`), Schaltflächen `Erstellen` (Absenden) und `Abbrechen`.
- **Beitreten-Formular**: das Formular mit dem zugänglichen Namen `Spielsitzung beitreten`
  im Dialog gleichen Namens, Eingabefeld `Sitzungscode` (`name="code"`), Schaltflächen
  `Beitreten` (Absenden) und `Abbrechen`. Weil der Hero ebenfalls eine Schaltfläche
  `Beitreten` trägt, ist das Absenden nur innerhalb des Formulars adressiert.
- **Offen** ist ein Formular, wenn sein Dialog im Dokument gerendert ist; **geschlossen**,
  wenn nicht. Höchstens einer der beiden Dialoge ist offen.
