...
// im `ref`-Element, reagiert auf Aenderungen von `grid`/`imageUrl` per `setGrid`/`setImage`
// und gibt die Canvas-Ressourcen im Cleanup genau einmal frei (spec.md Requirement
...
export function MapCanvas({ imageUrl, grid }: MapCanvasProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const handleRef = useRef<MapCanvasHandle | null>(null)

  // Nur beim Mounten erzeugen - spaetere Aenderungen von `grid`/`imageUrl` gehen ueber die
  // beiden Effekte darunter (`setGrid`/`setImage`), nicht ueber eine Neuerzeugung.
  useEffect(() => {
    ...
    let cancelled = false
    import('./canvas.js')
      .then(({ createMapCanvas }) => createMapCanvas(container, { imageUrl, grid }))
      .then((handle) => {
        if (cancelled) {
          handle.destroy()
          return
